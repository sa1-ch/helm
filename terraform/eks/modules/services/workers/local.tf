locals {
  ...
  aws_launch_template_name = "asg_instance_template"
  ...
  cluster_primary_security_group_id = var.cluster_version >= 1.14 ? element(concat([data.terraform_remote_state.eks_cluster.outputs.vpc_config[0].cluster_security_group_id], list("")), 0) : null
  cluster_security_group_id         = var.cluster_create_security_group ? join("", [data.terraform_remote_state.eks_cluster.outputs.vpc_config[0].cluster_security_group_id]) : var.cluster_security_group_id
   workers_group_defaults = merge(
    local.workers_group_defaults_default,
    var.workers_group_defaults,
  )

  workers_group_defaults_default = {
    instance_name                 = "tiger-mle-eks-instances" # default name for instances   
    ...
    root_volume_size              = "0"                       # root volume size of workers instances.
    root_volume_type              = "gp2"                       # root volume type of workers instances, can be 'standard', 'gp2', or 'io1'
    root_iops                     = "0"                         # The amount of provisioned IOPS. This must be set with a volume_type of "io1".
    key_name                      = "tiger-mle-ec2-keys"        # The key name that should be used for the instances in the autoscaling group
    ...
    additional_userdata           = "sudo yum install -y nfs-utils amazon-efs-utils"                          # userdata to append to the default userdata.
    ebs_optimized                 = true                        # sets whether to use ebs optimization on supported types.
    enable_monitoring             = true                        # Enables/disables detailed monitoring.
    public_ip                     = false                       # Associate a public ip address with a worker
    kubelet_extra_args            = "--node-labels=nodegroup-name=c5-4x-large-demand,instance-type=onDemand,nodegroup-type=stateful-workload"                          # This string is passed directly to kubelet if set. Useful for adding labels or taints.
