resource "aws_launch_template" "asg_instance_template"{
  ...
  iam_instance_profile {
    name = data.terraform_remote_state.iam_role.outputs.worker_instance_profile_id
  }
...
resource "aws_autoscaling_group" "smn" {
  ...
  mixed_instances_policy {
    launch_template {
          ...
          override {
                ...
                #security_group_names = [data.terraform_remote_state.eks_cluster.outputs.vpc_config[0].cluster_security_group_id]
          }
        }
  ...
  tags = concat(
    [
      ...
      {
        "key"                 = "k8s.io/cluster-autoscaler/${data.terraform_remote_state.eks_cluster.outputs.cluster_id}"
        "value"               = "owned"
        "propagate_at_launch" = true
      }
    ],
